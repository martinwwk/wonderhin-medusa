...
const ProductImageGrid: React.FC<GalleryProps> = ({data, variant = 'glass', className, galleryID="simpleGallery"}) => {
    // Stable gallery ID to prevent unnecessary re-renders
    const stableGalleryID = useMemo(() => galleryID || "simpleGallery", [galleryID]);

    useEffect(() => {
        const lightbox = new PhotoSwipeLightbox({
            gallery: `#${stableGalleryID}`,
            children: "a",
    ...
    }, [stableGalleryID]); // ✅ Only rerun if stable ID changes

    const galleryItems = useMemo(() => {
        return data?.gallery?.map((item: GalleryItem, index: number) => {
            const key = `${galleryID}-${index}`;
            const altText = `Product gallery ${item.id || index}`;
            const width = item.width || 630;
            const height = item.height || 835;
                        ...
                        <div key={key}>
                            <div className="mx-auto magnifier-image-container">
                                <SideBySideMagnifier
                                    src={item.original ?? productGalleryPlaceholder}
                                    largeSrc={
                                        item.largeImage ?? item.original ?? productGalleryPlaceholder
                                    }
                                    alt={altText}
                                />
                            </div>
    ...
    }, [data?.gallery, variant, galleryID]);
        ...
        <div
            ...
            className={cn(
                "pswp-gallery text-center",
                {
                    ...
                    "grid gird-cols-1 md:grid-cols-2 gap-5": variant === "innerZoom",
                },
                className
            )}
        >
