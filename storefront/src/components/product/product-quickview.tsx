...
export default function ProductQuickview() {
    ...
    if (!data || !('price' in data)) return null; // Guard clause based on Product shape
    const productData = data as Product;

    // Initialize attributes dynamically based on product variations
    const { initialAttributes} = useProductVariations(productData);
    const [attributes, setAttributes] = React.useState<{ [key: string]: string }>(initialAttributes);

    return (
        <div className="md:w-[600px] lg:w-[940px] xl:w-[1100px] mx-auto p-3  lg:p-7 bg-white rounded-md">
